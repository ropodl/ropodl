...
export async function useApiFetch<Res = unknown, Req = unknown>(
      ...
      ...(function () {
        ...
        if (Array.isArray(sortBy) && sortBy.length) {
          ...
          if (first && typeof first.key === "string") {
            return {
              sortBy: first.key,
              order: typeof first.order === "string" ? first.order : undefined,
  ...
  const executeRequest = async (): Promise<Res> => {
    try {
      const nitroOpts = fetchOptions as unknown as Parameters<typeof $fetch>[1];
  ...
  if (fetchOptions.responseType === "blob") {
    ...
    if (
      ...
      return new Blob([response as unknown as ArrayBuffer]) as unknown as Res;
    }
