<script setup>
  ...
  let works = [
    ...
    },
    {
      ...
      image: {
        thumbnail: "https://cdn.dribbble.com/users/1462542/screenshots/15078907/media/77d0f57efb1d67aeb81b8b520e143b36.png?compress=1&resize=768x576&vertical=top",
    ...
    },
    {
      ...
      image: {
        thumbnail: "https://cdn.dribbble.com/users/1462542/screenshots/13964553/media/de7681bf963d621cf0804f95d24f63f7.jpg",
    ...
    },
    {
      ...
      image: {
        thumbnail: "https://cdn.dribbble.com/userupload/5525097/file/original-bec38402c952bde85d074df812cabebc.png",
    ...
    },
    {
      ...
      image: {
        thumbnail: "https://cdn.dribbble.com/users/1462542/screenshots/6586732/legends.png",
  ...
  <v-container class="px-0 pb-16">
    ...
    <v-row justify="center">
      <v-col cols="10" class="position-relative">
        <v-card elevation="10" color="#3a3d4f" class="mb-10 position-sticky" :width="y >= 250 ? '700' : '100%'" style="top: 15px; backdrop-filter: blur(10px); z-index: 10">
          <v-tabs grow height="60" variant="tonal">
            <template v-for="category in categories">
              <v-tab rounded="0" class="text-lowercase" :color="current == category ? 'primary' : 'white'" variant="text" @click="current = category">{{ category }}</v-tab>
            </template>
          </v-tabs>
        </v-card>
        <v-row v-auto-animate>
          <template v-for="(work, i) in works">
            <v-col cols="12" sm="6" md="6" lg="4" v-if="work['category'] === current || current === 'All'">
              <v-dialog fullscreen persistent no-click-animation v-model="dialog.dialogs[i]" transition="fade-transition" scrim="black" width="auto" content-class="d-flex w-100">
                <template v-slot:activator="{ props: overlay }">
                  <v-card flat elevation="10" height="400" v-tilt="{ scale: 1.05, gyroscope: false }" @click="openDialog(i)" v-bind="{ overlay }">
                    <v-img cover class="w-100 h-100 align-end" :src="work['image'].thumbnail">
                      <v-card elevation="10" class="ma-4">
                        <v-card-text class="text-center" style="white-space: normal">
                          {{ work["title"] }}
                        </v-card-text>
                      </v-card>
                    </v-img>
                ...
                <div class="w-100 h-100">
                  <v-card flat color="rgba(0,0,0,0.8)" rounded="0" class="h-100 align-center justify-center" style="backdrop-filter: blur(2px)">
                    <div id="scene">
                      <v-img :src="works[i].image['full']" />
                    </div>
                    <template v-if="dialog.info">
                      <v-card border flat rounded="xl" width="400" class="position-absolute mx-auto" style="bottom: 20px; left: 0; right: 0">
                        <v-card-title class="text-wrap" v-text="work['title']"></v-card-title>
                        <v-card-text>
                          Category:
                          {{ work["category"] }}
                        </v-card-text>
                        <v-btn block variant="tonal" rounded="0" height="50" class="text-capitalize" target="_blank" :href="work['link']">
                          <v-icon start>
                            <Icon icon="mdi:link" />
                          </v-icon>
                          Open link
